...
public class Behaviour_Countdown : MonoBehaviour
{
	[Serializable]
	/// <summary>
	...
	[Header("Countdown")]
	[InfoBox("You can define the bounds of the countdown (how much it should count down from, and to how much, the format it should be displayed in (standard Unity float ToString formatting).")]
	...
	[ShowIf("FormatMethod" , FormatMethods.Explicit)]
	...
	[ShowIf("FormatMethod" , FormatMethods.Choices)]
	...
	[Header("Settings")]
	[InfoBox("You can choose whether or not the countdown should automatically start on its Start, at what frequency (in seconds) it should refresh (0 means every frame), and the countdown's speed multiplier " +
	...
	protected virtual void UpdateTime()
	{
		...
		{
			CurrentTime -= UnityEngine.Time.deltaTime * CountdownSpeed;
		}
		else
		{
			CurrentTime += UnityEngine.Time.deltaTime * CountdownSpeed;
	...
	{
		if (UnityEngine.Time.time - _lastRefreshAt > RefreshFrequency)
		{
			if (_text != null)
			{
				string newText = "";
				...
				{
					newText = FloatToTimeString(CurrentTime, Hours, Minutes, Seconds, Milliseconds);
				}
			...
			_lastRefreshAt = UnityEngine.Time.time;
		}
	}


	private string FloatToTimeString(float t, bool displayHours = false, bool displayMinutes = true, bool displaySeconds = true, bool displayMilliseconds = false)
	{
		int intTime = (int)t;
