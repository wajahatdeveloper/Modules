		...
		static ScriptOrderManager()
		{
			...
			if ((scripts.Count > 0) && !scripts.All(script => script.Value.IsValidExecutionOrder))
			{
				...
				catch (Exception e)
				{
					...
					Debug.LogError("Failed to resolve the script execution order dependencies. New script execution order values will not be applied.");
					return;
				}
			}
				...
				for (int i = island.Count - 1; i >= 0; --i)
				{
					...
					if (script.IsFixedOrder)
					{
						...
						if (script.Order > orderValue)
						{
							...
							throw new Exception(string.Format("Script {0} has a fixed execution order value of {1}, " +
							                                  "but other scripts following up have already been assigned an execution order" +
							                                  " lower than that.", script.Type.Name, script.Order));
						}
		...
		private class ScriptExecutionInfo : IComparable<ScriptExecutionInfo>
		{
			...
			public int Order
			{
				...
				set
				{
					if (IsFixedOrder)
					{
						throw new Exception("Cannot change the execution order of a script that is fixed.");
					}
			...
			public void AddDependency(ScriptExecutionInfo script)
			{
				if (script == this)
				{
					throw new Exception("Cannot let a script depend on itself.");
				}
		...
		private class GraphNode : IComparable<GraphNode>
		{
			...
			public void Visit(List<GraphNode> nodes, List<HashSet<GraphNode>> islands)
			{
				...
				else if (IsTemporary)
				{
					...
					cyclicPath.Add(errorNode.node.Type.Name);
					throw new Exception(String.Format("Cyclic script execution order dependencies detected: {0}.", string.Join(" -> ", cyclicPath)));
				}
