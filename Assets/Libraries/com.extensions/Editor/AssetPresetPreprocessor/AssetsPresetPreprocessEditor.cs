...
using UnityEngine;
	...
	public class AssetsPresetPreprocessEditor : Editor
	{
		...
		private static void SelectBase()
		{
			...
			if (presetBase == null)
			{
				presetBase = ScriptableObjectX.CreateAssetWithFolderDialog<AssetsPresetPreprocessBase>("AssetsPresetPostprocessBase");
			}
		...
		private void DrawPresetColourLine(Rect rect, Preset preset)
		{
			...
			Color color = EditorGUIX.Colors.Brown;
			if (preset == null) color = Color.red;
			else
			{
				var presetType = preset.GetTargetTypeName();
				if (presetType.Contains("Texture")) color = EditorGUIX.Colors.Blue;
				else if (presetType.Contains("Audio")) color = EditorGUIX.Colors.Red;
			}

			EditorGUIX.DrawColouredRect(cRect, color);
